...
- Steps: `ProtectedRoute`가 로그인 세션을 확인한다. `ProblemsPage`가 `useProblemList(page)`를 호출하고 hook이 `problemService.listProblems()`를 통해 `GET /api/v1/problems?page={page}`를 요청한다. hook은 API DTO를 `ProblemPage` view model로 변환하고 `ProblemTable`에 전달한다.
...
- Steps: `SubmitPage`가 `useProblemDetail(problemNumber)`로 `GET /api/v1/problems/{problemNumber}`를 호출해 문제 본문과 공개 예제를 표시한다. 같은 화면에서 `useMySubmissions(1, problemNumber)`로 해당 문제의 내 최근 제출을 조회한다. 사용자가 제출하면 `useCreateSubmission()`이 `POST /api/v1/submissions`를 호출한다. 생성된 제출 ID가 있으면 `useSubmissionDetail(submissionId)`가 `GET /api/v1/submissions/{submissionId}`를 호출해 채점 상태, 점수, 테스트 케이스 결과, 컴파일/런타임 오류 메시지를 조회한다. 제출 상태가 `QUEUED`, `RUNNING`, `JUDGING`, `PENDING`이면 2.5초 후 상세 조회를 반복한다.
...
- Side effects: 제출 성공 시 백엔드에 제출이 생성되고 초기 상태가 표시된다. 이후 비동기 채점 결과가 상세 조회 polling으로 갱신된다.
...
- Preconditions: 로그인 세션이 있어야 한다.
- Steps: `SubmissionsPage`가 `useMySubmissions(page)`를 호출하고 hook이 `submissionService.listMySubmissions()`로 `GET /api/v1/submissions/me`를 요청한다. API DTO는 `SubmissionPage` view model로 변환되어 테이블에 표시된다.
- Validation: page는 1 이상의 정수 상태로 관리한다. 백엔드가 page query parameter를 최종 검증한다.
...
- Steps: 비로그인 사용자가 접근하면 `ProtectedRoute`가 `/login`으로 이동시킨다. 로그인했지만 `ADMIN`이 아닌 사용자는 `ProtectedRoute requiredRole=ADMIN`이 `/problems`로 이동시켜 문제 생성 화면을 조립하지 않는다. 관리자는 `AdminProblemNewPage`에서 제목, 태그, 시간 제한, 메모리 제한, Markdown 본문, 선택 checker C++ 코드, 공개 예제 테스트 케이스, 실제 채점 테스트 케이스를 입력한다. 제출 시 page가 request DTO를 만들고 `useCreateProblem()`이 현재 access token으로 `problemService.createProblem()`을 호출해 `POST /api/v1/problems`를 요청한다.
- Validation: 프론트엔드는 제목 필수/20자 이하, 태그 필수/64자 이하, 시간/메모리 양의 정수, 본문 필수, 예제/실제 테스트 케이스 최소 1개를 확인한다. checker 코드가 공백이면 `checkerCode`를 요청 body에서 생략한다. checker 코드 형식, 입출력 배열 개수 일치, 관리자 권한은 백엔드가 최종 검증한다.
- Empty state: 빈 폼
